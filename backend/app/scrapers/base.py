...
CORE_SERVICE_KEYWORDS = [
    "检测", "监测", "勘察", "测绘", "鉴定", "试验", "检验",
    "定检", "年检", "观测",
]
# 注：「评估」从核心词移除 — 太宽泛，常被公司名（XX评估有限公司）、
# 咨询评估、市场评估等非检测场景污染。真正的工程评估（房屋安全鉴定评估、
# 节能评估、抗震鉴定）都由「鉴定」「检测」承载。
...
BLACKLIST_KEYWORDS = [
    ...
    # === 非检测服务 / 货物类（即使标题含「检测/监测」也要 LLM 二次判断）===
    # 货物采购 — 买设备/耗材/试剂本身，不是买检测服务
    "采购", "耗材", "试剂", "仪器", "设备", "装置", "配件", "备品",
    # 非检测服务 — 维护/保养/运维不是检测
    "维护服务", "维护保养", "维护项目", "运维", "保养", "修缮",
    # 安装/施工类 — 设备安装、工程施工不是检测服务
    "安装", "施工",
    # 设计/总承包类 — 设计合同（含「勘察设计」组合）不是检测服务
    "设计", "总承包", "EPC",
    # 消防气体灭火系统充装类 — 灭火剂充装/钢瓶逐年检定（设备充装维护，非工程检测）
    "灭火剂", "钢瓶", "充装", "瓶体",

    # === 白蚁防治 / 病虫害防治 / 消杀服务（房屋维护服务，非工程检测）===
    # 注：移除「评估」核心词后，这类标题通常已直接 reject；
    # 保留黑名单作防御，防止「白蚁防治检测」等组合混入。
    "白蚁防治", "白蚁", "防治服务", "病虫害防治", "消杀",

    # === 环保 / 生态 / 地质灾害类（不涉及）===
    # 即使含「检测/监测/评估」，主导也是环境监测而非土木工程实体检测
    "环保", "生态", "生态环境", "海洋生态", "养殖用海", "水质", "大气",
    "废气", "废液", "废水", "污水", "固废", "危废", "水土保持", "尾气",
    "油气回收", "空气检测", "碳排放", "污染", "排放", "流域", "河道治理",
    # 地质灾害 — 不是工程检测（我们做建筑物/市政/交通/水利实体检测）
    "地质灾害", "地灾",
    # 城市管理服务外包类 — 市容/城管/秩序管理
    "市容", "城管", "秩序管理", "管理服务外包",
    # 海事/航运服务（非码头土建结构检测）
    "通航", "海事", "航运",
    # 工业设施运营评估（电厂灰库、油库等）
...
# 更正/变更/补遗类后续公告：这些是对原招标公告的修正，不应作为新项目入库，
# 否则会与原项目重复（external_no 通常不同，去重兜底拦不住）。
CORRECTION_KEYWORDS = [
    "更正公告", "更正通知", "变更公告", "变更通知",
    "补遗公告", "补遗通知", "补遗文件",
    "澄清公告", "澄清通知", "澄清文件",
...
def is_correction_announcement(text: str) -> bool:
    """判断标题是否为更正/变更/补遗/澄清/延期类后续公告（应排除）。

    这类公告是对原招标公告的修正，external_no 通常与原公告不同，
    会作为重复项目入库。统一在 runner 层拦截。
    """
    if not text:
        return False
...
# 更正类关键词 → 提醒字样映射（用于 correction_notice 字段）
_CORRECTION_NOTICE_MAP = [
    ("更正", "有更正公告"),
    ("变更", "有变更公告"),
    ("补遗", "有补遗公告"),
    ("澄清", "有澄清公告"),
...
# 括号备注正则：（更正公告）/（变更通知）等
_PAREN_PAT = _re.compile(
    r"[（(][^）)]*(更正|变更|补遗|澄清|答疑|延期|撤销|撤回)[^）)]*[）)]"
)


def strip_correction_keyword(title: str) -> str:
    """从更正公告标题剥离「更正公告/变更通知」等词，得到核心项目名。

    例：
      "XX工程招标公告更正公告" → "XX工程招标公告"
      "XX工程检测（变更通知）" → "XX工程检测"
    """
    if not title:
        return ""
...
def derive_correction_notice(title: str) -> str:
    """根据标题命中的关键词，推导提醒字样。默认"有更正公告"。"""
    if not title:
        return "有更正公告"
    for kw, notice in _CORRECTION_NOTICE_MAP:
        if kw in title:
            return notice
    return "有更正公告"
