...
- **Auto-scrape**: `POST /api/scrape/run` (admin / bid_specialist) triggers `run_scraper(run_id, user_id)` via FastAPI BackgroundTasks. Iterates all registered scrapers (ccgp/ggzy/jhygcg/jhzjcs/jhggzy) via `ScraperRegistry.all()` (lazy-loaded, fault-tolerant). Prevents concurrent runs (409). Orphan runs cleaned on startup.
