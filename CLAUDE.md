...
- **Basic**: `keyword` (project name, with `keyword_match`="fuzzy"/"exact"), `status` (单选，向后兼容), `statuses` (多选，逗号分隔，非空时优先于 `status`，用 `ProjectInfo.status.in_(...)` 过滤), `bidding_type`
...
- **Column config persistence**: `ProjectList.vue` uses `watch(selectedColumnKeys)` (not `@change`) to save column config to localStorage. **多选模式下统一用单一全局 key** `project_list_columns_${uid}_all`（不再按状态分 tab，因为 activeStatus 现在是数组）。`loadColumnConfig('')` / `saveColumnConfig('', keys)` 都显式传空字符串。
- **List state preservation**: `ProjectList.vue` 通过 `sessionStorage['projectListPage']`（JSON: `{page, activeStatus, keyword, bidding_type, myProjects}`）保存列表状态。**`activeStatus` 现在是数组**（旧 string 格式在 `consumeListState` 中兼容：单 string 自动包装成 `[status]`）。`goToDetail(id)` 调用 `saveListState()` 后再 `router.push`。`onMounted` 用 `consumeListState()` 读取并清除。`Layout.vue` 侧边栏点击 `/projects` 时 `sessionStorage.removeItem('projectListPage')` — 即「侧边栏点击 = 回到默认状态，浏览器后退 = 保留状态」。`_initialized` 标志位配合 `nextTick` 防止高级筛选恢复期间 watch 异步重置页码。
- **List UI 细节 (状态多选)**: 单页 15 条。**状态快捷按钮是 toggle 多选**，不是单选。`activeStatus: ref([...DEFAULT_STATUSES])` 默认勾选 9 个（跟进中/已发公告/未报名/已报名/准备投标/已投标/已中标/未中标/已流标，**不含已放弃**）。按钮样式保留 el-button 原配色（已放弃 info 色、已中标 success、未中标 danger、已流标 warning、其余 primary），未勾选时 `type=''`。首按钮是「全选/全取消」toggle：满 10 个 → 显示「全取消」（清空数组）；未满 → 显示「全选」（全选 10 个含已放弃）。`loadData()` 数组为空时直接置空表格不发请求；非空时用 `activeStatus.join(',')` 传 `statuses` 给后端。**不再用 `exclude_abandoned`** —— 未勾选已放弃 = 它不在数组里，天然过滤。
...
- **Document parsing (招标公告)**: `POST /api/documents/parse?parse_type=bidding&project_id=N` uploads file → Qwen-Long extracts fields → auto-creates bidding_unit/agency/platform orgs via `_match_or_create_org` (with `org_type=external`) → returns form fields for frontend to apply. File metadata appended to `bid_documents` on save. System prompt in `document_parser.py:SYSTEM_PROMPT` asks LLM to extract bidding fields; `FIELD_ALIASES` maps Chinese keys to English. **保证金字段提取**：SYSTEM_PROMPT 含「是否有投标保证金」+「保证金金额」，FIELD_ALIASES 映射到 `has_deposit` / `deposit_amount`。**地区结构化**：`_build_bidding_response` 调用 `scrapers/base.py:extract_region_from_text(region_text)` 把中文字符串转成数组 `region`，与 `region_text` 并存于响应 `fields`。
- **Submit-time validation (前端 only)**: `ProjectDetail.vue:validateBeforeSubmit()` 在 `handleSubmit()` 里、`ensureSpecialistBeforeSave()` 之前执行，三条规则任一失败即 `return` 不推进：(1) `projectForm.region` 数组非空 + `projectForm.manager_ids` 数组非空；(2) `bidForm.bid_method !== '独立'` 时 `bidForm.partner_ids` 必须非空；(3) `bidForm.has_deposit === true` 时 `deposit_amount > 0` + `deposit_date` 非空 + `deposit_status` 非「无」。后端不兜底，旧数据推进时会被卡住（预期行为）。
- **Bid info early persistence**: `collectSaveData()` 在初始 `data` 对象里始终带上 `has_deposit` + `deposit_amount`（与 `bid_specialist_id` 同级），让招标文件解析设置的保证金字段在状态推进到「准备投标」之前也能持久化（投标卡片在准备投标之前不可见，正常 Object.assign 不会带上这两个字段）。`applyParsedFields` 解析后写入 `bidForm.has_deposit` / `deposit_amount`，但**不动** `deposit_date` / `deposit_status`（这俩是公司内部信息，招标文件没有）。
...
- **我的项目 filter**: `ProjectList.vue` shows "我的项目" toggle button for `bid_specialist` role only. **默认勾选**（`myProjects = ref(userStore.user?.role === 'bid_specialist')`）。Toggles `myProjects` ref → calls `/api/projects` with `bid_specialist_id=<self> + include_unassigned_specialist=true`（不再叠加 `exclude_abandoned`，由 `activeStatus` 数组天然过滤）。So "我的项目" = 当前专员 + 未指派，叠加状态多选过滤。
