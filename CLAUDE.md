...
- **Scraper architecture**: `scrapers/base.py:BaseScraper` (abstract `fetch`/`normalize`, `fetch_with_retry` with 3 retries + exponential backoff, error message includes exception type). `ScraperRegistry` uses `importlib.import_module` with try/except per scraper. All four site modules use `requests` (no Playwright runtime dependency): `ccgp.py` (scrapling + curl_cffi browser fingerprint), `ggzy.py` (requests + WAF bypass via session cookie warmup), `jhygcg.py` (requests POST JSON API), `jhzjcs.py` (requests POST JSON API, direct `getComparePublicList` call).
- **Result announcement filter**: `scrapers/base.py:RESULT_KEYWORDS` + `is_result_announcement()` excludes 中标/成交/开标/结果/废标/流标/终止/合同 公告/公示/记录. All four scrapers call this in `normalize()` to keep only 招标公告.
